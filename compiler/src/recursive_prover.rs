...
use crate::groth16_native::{
    prove as groth16_prove, verify as groth16_verify, Groth16Params, ZKProof,
};
use crate::r1cs::R1CSSystem;
use crate::solidity_verifier::{generate_solidity_verifier, VerifierCoordinates};
use ark_bn254::Fr;
...
fn folding_challenge(a: &ZKProof, b: &ZKProof) -> Fr {
    ...
    for pi in &a.public_inputs {
        t.absorb_bytes(&pi.to_bytes_le());
    }
    for pi in &b.public_inputs {
        t.absorb_bytes(&pi.to_bytes_le());
    }
    t.challenge()
}
...
pub fn fold_instances(
    ...
    let folded_pub: Vec<BigUint> = a
        .public_input
        .iter()
        .zip(b.public_input.iter())
        .map(|(x, y)| {
            let r_term = {
                let bytes = x.to_bytes_le();
                (Fr::from_le_bytes_mod_order(&bytes) * r)
                    .into_bigint()
                    .to_bytes_le()
            };
            let omr_term = {
                let bytes = y.to_bytes_le();
                (Fr::from_le_bytes_mod_order(&bytes) * one_minus_r)
                    .into_bigint()
                    .to_bytes_le()
            };
...
pub fn fold_many(instances: &[IVCInstance], params: &Groth16Params) -> Result<FoldedProof, String> {
...
pub fn prove_recursive_production(
    ...
    for (i, input) in inputs.iter().enumerate() {
        ...
        for (k, v) in input {
            step_private.insert(k.clone(), v.clone());
        }
...
pub fn batch_verify(proofs: &[ZKProof], params: &Groth16Params) -> Result<bool, String> {
    ...
    for (i, p) in proofs.iter().enumerate() {
        ...
        if !groth16_verify(params, p).map_err(|e| format!("batch_verify[{}]: {}", i, e))? {
            return Ok(false);
        }
    }
    Ok(true)
...
mod tests {
    ...
    fn test_fold_invalid_rejected() {
        ...
        assert!(
            result.is_err(),
            "Tampered proof must be rejected during fold"
        );
    }
    ...
    fn test_recursive_production() {
        ...
        let result = prove_recursive_production(&rcs, &params, &initial, &inputs).unwrap();
    ...
    fn test_batch_verify_rejects_invalid() {
        ...
        assert!(
            ...
            "Tampered proof must not batch-verify as true, got {:?}",
            result
        );
    }
    ...
    fn test_mismatched_input_lengths() {
        ...
        let proof_b =
            crate::groth16_native::prove(&rcs_b, &params_b, priv_b, HashMap::new()).unwrap();
        let b = IVCInstance {
            step: 1,
        ...
        assert!(
            result.is_err(),
            "Folding instances from different circuits must fail, got {:?}",
            result
        );
    }
}
