...
use crate::groth16_native::{Groth16Params, ZKProof, setup as groth16_setup, prove as groth16_prove, verify as groth16_verify};
...
impl ZKMLCircuit {
    ...
    pub fn build_r1cs(model: &ZKModel, cs: &mut R1CSSystem) -> Result<Self, String> {
        ...
        for (li, layer) in model.layers.iter().enumerate() {
            match layer {
                ZKLayer::Dense { weights, bias, activation } => {
                    let output_dim = weights.len();
                    let input_dim = if weights.is_empty() { 0 } else { weights[0].len() };
                    
                    if input_dim != current.len() {
                        return Err(format!(
                            "Layer {}: weight input dim {} != current activation dim {}",
                            li, input_dim, current.len()
                        ));
                    }
                    ...
                    for o in 0..output_dim {
                        ...
                        if bias_val > zero { c_terms.push(("ONE".to_string(), bias_val)); }
                ...
                ZKLayer::Softmax => {
                    ...
                    let scale = BigUint::from(model.scale);
                    let mut sum_terms = Vec::new();
                    for var in &softmax_vars {
                        let vname = cs.vars.iter().find(|(_,v)| v.0 == var.0).map(|(n,_)| n.clone()).unwrap_or_default();sum_terms.push((vname, BigUint::from(1u64)));
                    }
                    let sum_var_name = format!("softmax_sum");
    ...
    pub fn forward(&self, input: &[i32]) -> Result<Vec<BigUint>, String> {
        if input.len() != self.model.input_dim {
            return Err(format!(
                ...
                self.model.input_dim, input.len()
            ));
        }
    ...
    pub fn estimate_constraints(&self) -> usize {
        ...
        for layer in &self.model.layers {
            match layer {
                ZKLayer::Dense { weights, activation, .. } => {
                    let output_dim = weights.len();
    ...
    ZKModel {
        ...
        layers: vec![
            ZKLayer::Dense {
                ...
                bias: vec![-20, -20, -30, -20, -20, -30, -20, -20, -20, -20, -20, -30, -20, -20, -20, -20],
                activation: Activation::ReLU,
            },
            ZKLayer::Dense {
...
pub fn prove_inference(
    model: &ZKModel,
    input: &[i32],
) -> Result<(ZKMLProof, R1CSSystem), String> {
    // 1. Build the circuit
    ...
    match groth16_setup(&cs) {
        Ok(params) => {
            match groth16_prove(&cs, &params, witness.clone(), HashMap::new()) {
                Ok(proof) => {
...
pub fn generate_report(proof: &ZKMLProof) -> String {
    ...
    report.push_str(&format!("=== ZKML Proof Report: {} ===\n\n", proof.model_name));
    ...
    report.push_str(&format!("\n## Performance\n\n"));
    report.push_str(&format!("Circuit size: {} constraints\n", proof.total_constraints));
    report.push_str(&format!("Est. proving time (Groth16): {:.1}s\n", proof.total_constraints as f64 * 0.0003));
    report.push_str(&format!("Est. verification time: ~5ms\n"));
    report.push_str(&format!("Proof size: 128 bytes (constant)\n"));
...
mod tests {
    ...
    fn test_different_inputs_different_outputs() {
        ...
        let all_same = out1.iter().zip(out2.iter()).all(|(a, b)| a == b);
    ...
    fn test_mnist_large_inputs_class_0() {
        ...
        assert!(output[0] > output[1], "Expected class 0 to win for large input [50,50,50,50]");
    }
    ...
    fn test_mnist_small_inputs_class_1() {
        ...
        assert!(output[1] > output[0], "Expected class 1 to win for small input [1,1,1,1]");
    }
    ...
    fn test_credit_good_profile_high_score() {
        ...
        assert!(score > threshold, "Expected credit score > 50 for good profile, got raw: {}", score);
    }
    ...
    fn test_credit_bad_profile_low_score() {
        ...
        assert!(score < threshold, "Expected credit score < 30 for bad profile, got raw: {}", score);
    }
}
