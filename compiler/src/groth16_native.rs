...
fn enforce_constraints(
    ...
    for c in &r1cs.constraints {
        fn build_lc(
            ...
            terms
                .iter()
                .fold(LinearCombination::zero(), |lc, (vi, coeff)| {
...
pub fn setup(r1cs: &R1CSSystem) -> Result<Groth16Params, String> {
    ...
    let mut pkb = vec![];
    pk.serialize_compressed(&mut pkb)
        .map_err(|e| format!("PK ser: {e}"))?;
    let mut vkb = vec![];
    vk.serialize_compressed(&mut vkb)
        .map_err(|e| format!("VK ser: {e}"))?;
    Ok(Groth16Params {
        pk: pkb,
        vk: vkb,
        vk_coords,
...
pub fn prove(
    ...
    let c = WitnessCircuit {
        r1cs: r1cs.clone(),
        inputs: witness,
    };
    let pk: ark_groth16::ProvingKey<Bn254> =
        CanonicalDeserialize::deserialize_compressed(&params.pk[..])
    ...
    let mut pb = vec![];
    proof
        .serialize_compressed(&mut pb)
        .map_err(|e| format!("Proof ser: {e}"))?;
    ...
    Ok(ZKProof {
        proof: pb,
        public_inputs: pub_inp,
    })
}

pub fn verify(params: &Groth16Params, proof: &ZKProof) -> Result<bool, String> {
    let vk: ark_groth16::VerifyingKey<Bn254> =
        CanonicalDeserialize::deserialize_compressed(&params.vk[..])
            .map_err(|e| format!("VK deser: {e}"))?;
    let prf: ark_groth16::Proof<Bn254> =
        CanonicalDeserialize::deserialize_compressed(&proof.proof[..])
            .map_err(|e| format!("Proof deser: {e}"))?;
    let pvk = ark_groth16::prepare_verifying_key(&vk);
    let pub_s: Vec<Fr> = proof.public_inputs.iter().map(bu_to_fr).collect();
...
mod tests {
    ...
    fn test_wrong_witness_rejected() {
        ...
        assert!(
            result.is_err(),
            "Contradictory witness must be rejected by prove()"
        );
        assert!(
            result.unwrap_err().contains("Constraint"),
    ...
    fn test_zero_knowledge_property() {
        ...
        let has_secret = proof
            .public_inputs
            .iter()
            .any(|bu| *bu == BigUint::from(99999u64));
