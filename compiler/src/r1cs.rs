...
fn one() -> BigUint { BigUint::from(1u64) }
pub(crate) fn zero() -> BigUint { BigUint::from(0u64) }
pub(crate) fn coeff(c: u64) -> BigUint { BigUint::from(c) }
...
impl R1CSSystem {
  ...
  pub fn add_constraint(
    ...
    let a_vec: Vec<(usize, BigUint)> = a.iter().map(|(n, coeff)| (self.alloc_witness(n).0, coeff.clone())).collect();
    let b_vec: Vec<(usize, BigUint)> = b.iter().map(|(n, coeff)| (self.alloc_witness(n).0, coeff.clone())).collect();
    let c_vec: Vec<(usize, BigUint)> = c.iter().map(|(n, coeff)| (self.alloc_witness(n).0, coeff.clone())).collect();
    self.constraints.push(SparseConstraint { a: a_vec, b: b_vec, c: c_vec });
  }

  pub fn add_constraint_u64(&mut self, a: &[(String, u64)], b: &[(String, u64)], c: &[(String, u64)]) {
    self.add_constraint(
      &a.iter().map(|(n, c)| (n.clone(), coeff(*c))).collect::<Vec<_>>(),
      &b.iter().map(|(n, c)| (n.clone(), coeff(*c))).collect::<Vec<_>>(),
      &c.iter().map(|(n, c)| (n.clone(), coeff(*c))).collect::<Vec<_>>(),
    );
  }
  ...
  pub fn num_vars(&self) -> usize { self.next_var }
  ...
  pub fn solve_witness(
    ...
    for _round in 0..200 {
      ...
      for c in &self.constraints {
        ...
        if c.a.len() == 1 && c.b.len() == 1 && c.c.len() == 1
          && c.a[0].0 == c.b[0].0 && c.a[0].0 == c.c[0].0
        {
          continue;
        }
        ...
        if c.a.len() == 1 && c.b.len() == 1 && c.c.len() == 1
          && c.a[0].1 == one() && c.b[0].1 == one() && c.c[0].1 == one()
        {
          let bit_i = c.a[0].0;
          let dense_i = c.b[0].0;
          let relu_i = c.c[0].0;
        ...
        let c_is_zero = c.c.is_empty()
          || (c.c.len() == 1 && c.c[0].1 == zero() && c.c[0].0 == 0);
        if c_is_zero {
        ...
        if ak && bk {
          ...
          for (idx, _) in &c.c {
            if !known.contains(idx) {
              if uk.is_some() { single = false; break; }
              uk = Some(*idx);
            }
          }
          if let Some(ci) = uk {
            let tgt = mod_mul(&av, &bv);
            let rest: BigUint = c.c.iter()
              .filter(|(idx, _)| *idx != ci)
              .fold(zero(), |acc, (idx, tc)| mod_add(&acc, &mod_mul(tc, &w[*idx])));
            let num = mod_sub(&tgt, &rest);
            let cc = c.c.iter().find(|(idx, _)| *idx == ci).map(|(_, c)| c.clone()).unwrap_or(one());
            if cc == one() {
              w[ci] = num; known.insert(ci); changed = true;
            } else if let Some(inv) = mod_inv(&cc) {
              w[ci] = mod_mul(&num, &inv); known.insert(ci); changed = true;
            }
          }
        }
        ...
        if bk && ck && bv != zero() {
          ...
          if let Some(ai) = uk {
            let rest: BigUint = c.a.iter()
              .filter(|(idx, _)| *idx != ai)
              .fold(zero(), |acc, (idx, tc)| mod_add(&acc, &mod_mul(tc, &w[*idx])));
            let ca = c.a.iter().find(|(idx, _)| *idx == ai).map(|(_, c)| c.clone()).unwrap_or(one());
            if let Some(b_inv) = mod_inv(&bv) {
              let tgt = mod_mul(&cv, &b_inv);
              let num = mod_sub(&tgt, &rest);
              if ca == one() { w[ai] = num; known.insert(ai); changed = true; }
              else if let Some(inv) = mod_inv(&ca) { w[ai] = mod_mul(&num, &inv); known.insert(ai); changed = true; }
            }
          }
        }
        ...
        if ak && ck && av != zero() {
          ...
          if let Some(bi) = uk {
            let rest: BigUint = c.b.iter()
              .filter(|(idx, _)| *idx != bi)
              .fold(zero(), |acc, (idx, tc)| mod_add(&acc, &mod_mul(tc, &w[*idx])));
            let cb = c.b.iter().find(|(idx, _)| *idx == bi).map(|(_, c)| c.clone()).unwrap_or(one());
            if let Some(a_inv) = mod_inv(&av) {
              let tgt = mod_mul(&cv, &a_inv);
              let num = mod_sub(&tgt, &rest);
              if cb == one() { w[bi] = num; known.insert(bi); changed = true; }
              else if let Some(inv) = mod_inv(&cb) { w[bi] = mod_mul(&num, &inv); known.insert(bi); changed = true; }
            }
          }
        }
      }
    ...
    for c in &self.constraints {
      if c.a.len() != 1 || c.a[0].1 != one() { continue; }
      if c.b.len() != 1 || c.b[0].0 != 0 || c.b[0].1 != one() { continue; }
      if c.c.len() <= 1 { continue; }

      let sig_i = c.a[0].0;
      if !known.contains(&sig_i) { continue; }
...
mod tests {
  ...
  fn bu(n: u64) -> BigUint { BigUint::from(n) }
