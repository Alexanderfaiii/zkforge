...
pub struct PlonkProvingKey {
    pub srs: PlonkSRS,
    pub ql: DensePolynomial<Fr>,
    pub qr: DensePolynomial<Fr>,
    pub qo: DensePolynomial<Fr>,
    pub qm: DensePolynomial<Fr>,
    pub qc: DensePolynomial<Fr>,
    pub sigma_1: DensePolynomial<Fr>,
    pub sigma_2: DensePolynomial<Fr>,
    pub sigma_3: DensePolynomial<Fr>,
...
pub struct PlonkVerifyingKey {
    pub ql_comm: G1Affine,
    pub qr_comm: G1Affine,
    pub qo_comm: G1Affine,
    pub qm_comm: G1Affine,
    pub qc_comm: G1Affine,
    pub s1_comm: G1Affine,
    pub s2_comm: G1Affine,
    pub s3_comm: G1Affine,
    pub g2_x: G2Affine,
    pub num_public: usize,
    pub num_gates: usize,
...
pub struct PlonkProof {
    ...
    pub qm_eval: Fr,
    pub qc_eval: Fr,
    pub linearisation_eval: Fr,
    pub w_z_comm: G1Affine,
    pub w_z_w_comm: G1Affine,
    pub t_eval: Fr,
}

fn bu2fr(bu: &BigUint) -> Fr {
...
pub fn generate_srs_from_tau(tau: Fr, max_degree: usize) -> PlonkSRS {
    ...
    debug_assert!(
        srs.validate().is_ok(),
        "SRS internal consistency check failed"
    );
    srs
}
...
fn commit_poly(poly: &DensePolynomial<Fr>, srs: &PlonkSRS) -> G1Affine {
    ...
    for (i, c) in poly.coeffs.iter().enumerate() {
        if i < srs.g1_powers.len() && !c.is_zero() {
            r = (r + srs.g1_powers[i] * *c).into();
        }
    }
    r
}
...
fn var_name(r1cs: &R1CSSystem, idx: usize) -> String {
    r1cs.vars
        .iter()
        .find(|(_, v)| v.0 == idx)
        .map(|(n, _)| n.clone())
...
fn build_permutation(
    ...
    for (i, (an, bn, cn)) in wires.iter().enumerate() {
        if !an.is_empty() {
            first_a.entry(an.clone()).or_insert(i);
        }
        if !bn.is_empty() {
            first_b.entry(bn.clone()).or_insert(i);
        }
        if !cn.is_empty() {
            first_c.entry(cn.clone()).or_insert(i);
        }
    }
    ...
    for i in 0..n {
        let an = if i < wires.len() {
            &wires[i].0
        } else {
            &String::new()
        };
        let bn = if i < wires.len() {
            &wires[i].1
        } else {
            &String::new()
        };
        let cn = if i < wires.len() {
            &wires[i].2
        } else {
            &String::new()
        };
        let ja = if an.is_empty() {
            i
        } else {
            *first_a.get(an).unwrap_or(&i)
        };
        let jb = if bn.is_empty() {
            i
        } else {
            *first_b.get(bn).unwrap_or(&i)
        };
        let jc = if cn.is_empty() {
            i
        } else {
            *first_c.get(cn).unwrap_or(&i)
        };
...
fn quotient_division(poly: &DensePolynomial<Fr>, point: Fr) -> DensePolynomial<Fr> {
    ...
    while result.last().is_some_and(|x| x.is_zero()) {
        result.pop();
    }
...
pub fn setup(
    r1cs: &R1CSSystem,
    srs: &PlonkSRS,
) -> Result<(PlonkProvingKey, PlonkVerifyingKey), String> {
    let nc = r1cs.constraints.len();
    let ds = (nc * 4).next_power_of_two();
    let domain = Radix2EvaluationDomain::<Fr>::new(ds).ok_or("domain failed")?;
    if domain.size() > srs.g1_powers.len() {
        return Err("SRS too small".to_string());
    }
    let n = domain.size();
    let mut ql = vec![Fr::zero(); n];
    let mut qr = vec![Fr::zero(); n];
    let mut qo = vec![Fr::zero(); n];
    let mut qm = vec![Fr::zero(); n];
    ...
    for (i, c) in r1cs.constraints.iter().enumerate() {
        ...
        if c.a.len() == 1 && c.b.len() == 1 && c.c.len() == 1 {
            qm[i] = Fr::one();
            qo[i] = -Fr::one();
        } else if c.a.len() == 2 && c.b.len() == 1 && c.c.len() == 1 {
            ql[i] = Fr::one();
            qr[i] = Fr::one();
            qo[i] = -Fr::one();
        } else {
    ...
    let qlp = DensePolynomial::from_coefficients_vec(ql);
    let qrp = DensePolynomial::from_coefficients_vec(qr);
    let qop = DensePolynomial::from_coefficients_vec(qo);
    let qmp = DensePolynomial::from_coefficients_vec(qm);
    ...
    Ok((
        PlonkProvingKey {
            ...
            domain,
            num_gates: nc,
            num_public: r1cs.public_vars.len(),
            wires: vec![],
        },
        PlonkVerifyingKey {
            ql_comm: commit_poly(&qlp, srs),
            qr_comm: commit_poly(&qrp, srs),
            qo_comm: commit_poly(&qop, srs),
            qm_comm: commit_poly(&qmp, srs),
            qc_comm: commit_poly(&qcp, srs),
            s1_comm: commit_poly(&sigma_1, srs),
            s2_comm: commit_poly(&sigma_2, srs),
            s3_comm: commit_poly(&sigma_3, srs),
            g2_x: srs.g2_powers[1],
            num_public: r1cs.public_vars.len(),
            num_gates: nc,
            domain_size: n,
        },
...
pub fn build_wires_from_r1cs(
    r1cs: &R1CSSystem,
    private: &HashMap<String, BigUint>,
    public: &HashMap<String, BigUint>,
    nw: usize,
) -> Result<(Vec<Fr>, Vec<Fr>, Vec<Fr>), String> {
    let mut a = Vec::with_capacity(nw);
    let mut b = Vec::with_capacity(nw);
    let mut c = Vec::with_capacity(nw);
    let mut assignments = private.clone();
    assignments.insert("ONE".into(), BigUint::from(1u64));
    for (n, v) in public {
        assignments.insert(n.clone(), v.clone());
    }
    let witness = r1cs
        .solve_witness(&assignments)
        .map_err(|e| format!("Witness solver: {e}"))?;

    // Reject if any constraint is violated with provided witness
    let m_str = crate::r1cs::BN254_SCALAR_FIELD;
    let field_mod =
        BigUint::parse_bytes(m_str.as_bytes(), 10).unwrap_or_else(|| BigUint::from(0u64));
    for (i, c) in r1cs.constraints.iter().enumerate() {
        ...
        let bv = eval_lc(&c.b);
        let cv = eval_lc(&c.c);
        let cv_mod = cv.clone() % &field_mod;
        if (&av * &bv) % &field_mod != cv_mod {
            let an =
                c.a.first()
                    .map(|(vi, _)| var_name(r1cs, *vi))
                    .unwrap_or_default();
...
pub fn prove(
    pk: &PlonkProvingKey,
    r1cs: &R1CSSystem,
    priv_in: &HashMap<String, BigUint>,
    pub_in: &HashMap<String, BigUint>,
) -> Result<PlonkProof, String> {
    ...
    if !z_vals[n - 1].is_zero() && z_vals[n - 1] != Fr::one() {
        return Err(format!(
            "Permutation product did not telescope to 1: {:?}",
            z_vals[n - 1]
        ));
    }
    ...
    let qe: Vec<Fr> = (0..n)
        .map(|i| {
            ...
            let gv = pk.ql.evaluate(&r) * a_vals[i]
                + pk.qr.evaluate(&r) * b_vals[i]
                + pk.qo.evaluate(&r) * c_vals[i]
                + pk.qm.evaluate(&r) * a_vals[i] * b_vals[i]
                + pk.qc.evaluate(&r);
    ...
    let sp = n / 3;
    let t_lo =
        DensePolynomial::from_coefficients_vec(tp.coeffs[..sp.min(tp.coeffs.len())].to_vec());
    let ms = sp.min(tp.coeffs.len());
    let me = (2 * sp).min(tp.coeffs.len());
    let t_mid = if ms < tp.coeffs.len() {
        DensePolynomial::from_coefficients_vec(tp.coeffs[ms..me].to_vec())
    } else {
        DensePolynomial::zero()
    };
    let hs = (2 * sp).min(tp.coeffs.len());
    let t_hi = if hs < tp.coeffs.len() {
        DensePolynomial::from_coefficients_vec(tp.coeffs[hs..].to_vec())
    } else {
        DensePolynomial::zero()
    };
    ...
    let (s1e, s2e, s3e) = (
        pk.sigma_1.evaluate(&z_chal),
        pk.sigma_2.evaluate(&z_chal),
        pk.sigma_3.evaluate(&z_chal),
    );
    let ze = zp.evaluate(&z_chal);
    let zwe = zp.evaluate(&(z_chal * omega));
    let (qle, qre, qoe, qme, qce) = (
        pk.ql.evaluate(&z_chal),
        pk.qr.evaluate(&z_chal),
        pk.qo.evaluate(&z_chal),
        pk.qm.evaluate(&z_chal),
        pk.qc.evaluate(&z_chal),
    );
    ...
    for (i, c) in ap.coeffs.iter().enumerate() {
        fc[i] += *c;
    }
    for (i, c) in bp.coeffs.iter().enumerate() {
        fc[i] += *c * v;
    }
    for (i, c) in cp.coeffs.iter().enumerate() {
        fc[i] += *c * v2;
    }
    while fc.last().is_some_and(|x| x.is_zero()) {
        fc.pop();
    }
    ...
    Ok(PlonkProof {
        ...
        qm_eval: qme,
        qc_eval: qce,
        linearisation_eval: Fr::zero(),
        w_z_comm: commit_poly(&wz, &pk.srs),
        w_z_w_comm: commit_poly(&wzw, &pk.srs),
        t_eval: te,
    })
}
...
pub fn verify(vk: &PlonkVerifyingKey, proof: &PlonkProof, pi: &[Fr]) -> Result<bool, String> {
    if proof.a_comm.is_zero()
        && proof.b_comm.is_zero()
        && proof.c_comm.is_zero()
        && vk.num_gates > 0
    {
        return Ok(false);
    }
    ...
    if zh * proof.t_eval != gate + pi_at_zeta {
        return Ok(false);
    }
    ...
    if lhs != rhs {
        return Err("KZG wire batch opening failed: lhs != rhs".to_string());
    }
    ...
    if z_lhs != z_rhs {
        return Err("KZG z opening failed at zeta*omega: lhs != rhs".to_string());
    }
...
#[allow(dead_code)]
fn lagrange_i(vk: &PlonkVerifyingKey, z: Fr, i: usize) -> Fr {
    let n = Fr::from(vk.domain_size as u64);
    let zh = z.pow([vk.domain_size as u64]) - Fr::one();
    let w = Fr::get_root_of_unity(vk.domain_size as u64).unwrap_or(Fr::from(7u64));
    let wi = w.pow([i as u64]);
    zh * wi * (n * (z - wi)).inverse().unwrap_or(Fr::zero())
}

impl PlonkProvingKey {
    pub fn to_bytes(&self) -> Vec<u8> {
        let mut b = vec![];
        CanonicalSerialize::serialize_compressed(&self.ql, &mut b).ok();
        b
    }
}
impl PlonkVerifyingKey {
    pub fn to_bytes(&self) -> Vec<u8> {
        let mut b = vec![];
        CanonicalSerialize::serialize_compressed(&self.ql_comm, &mut b).ok();
        b
    }
}
impl PlonkProof {
    pub fn to_bytes(&self) -> Vec<u8> {
        let mut b = vec![];
        CanonicalSerialize::serialize_compressed(&self.a_comm, &mut b).ok();
        CanonicalSerialize::serialize_compressed(&self.b_comm, &mut b).ok();
        CanonicalSerialize::serialize_compressed(&self.c_comm, &mut b).ok();
        b
    }
}
...
mod tests {
    ...
    #[test]
    fn srs() {
        let s = generate_srs(64);
        assert_eq!(s.g1_powers.len(), 65);
    }
    #[test]
    fn setup_ok() {
        ...
        r.alloc_witness("x");
        r.alloc_witness("y");
        r.add_mul_constraint("z", "x", "y");
        let s = generate_srs(1024);
        let (p, v) = setup(&r, &s).unwrap();
        assert_eq!(p.num_gates, 1);
        assert!(v.domain_size >= 4);
    }
    #[test]
    fn prove_verify_single_gate() {
        let mut r = R1CSSystem::new();
        r.alloc_public("z");
        r.alloc_witness("x");
        r.alloc_witness("y");
        ...
        pi.insert("x".into(), BigUint::from(3u64));
        pi.insert("y".into(), BigUint::from(4u64));
        let proof = prove(&p, &r, &pi, &HashMap::new()).unwrap();
        let ok = verify(&v, &proof, &[]);
        assert!(ok.is_ok(), "verify error: {:?}", ok);
        assert!(ok.unwrap(), "proof verification failed");
    }
    #[test]
    fn tampered_rejected() {
        ...
        pi.insert("x".into(), BigUint::from(3u64));
        pi.insert("y".into(), BigUint::from(4u64));
        let mut proof = prove(&p, &r, &pi, &HashMap::new()).unwrap();
        proof.ql_eval -= Fr::one();
        assert!(!verify(&v, &proof, &[]).unwrap_or(false));
    }
    #[test]
    fn serial() {
        let mut r = R1CSSystem::new();
    ...
    fn test_three_gate_tampered_permutation() {
        ...
        proof.a_eval += Fr::one();
        assert!(
            !verify(&vk, &proof, &[]).unwrap_or(true),
