...
fn native_verify(
    ...
    let mut pk = [0u8; 65];
    pk[0] = 0x04;
    pk[1..33].copy_from_slice(px);
    pk[33..65].copy_from_slice(py);
    ...
    let mut sb = [0u8; 64];
    sb[..32].copy_from_slice(sr);
    sb[32..].copy_from_slice(ss);
    let sig = match Signature::from_slice(&sb) {
        Ok(s) => s,
        Err(_) => return false,
    };
...
pub fn hex_to_bytes32(hex_str: &str) -> Result<[u8; 32], String> {
    ...
    if bytes.len() > 32 {
        return Err(format!("Hex too long: {} bytes", bytes.len()));
    }
    let mut out = [0u8; 32];
...
pub fn biguint_to_bytes32(val: &str) -> Result<[u8; 32], String> {
    ...
    if bytes.len() > 32 {
        return Err("Value > 32 bytes".to_string());
    }
    let mut out = [0u8; 32];
    let start = 32 - bytes.len();
    out[start..].copy_from_slice(&bytes);
...
pub fn generate_test_vector(
    ...
    let sk_bytes: [u8; 32] = [
        0x8b, 0x4a, 0x3f, 0x17, 0x2d, 0x5b, 0x6e, 0x89, 0x7c, 0x3f, 0x2a, 0xbb, 0x9e, 0x1d, 0x4f,
        0x06, 0xcd, 0x8a, 0x2d, 0x7e, 0x3b, 0x5c, 0x1f, 0x42, 0xaa, 0x8b, 0x6e, 0x3d, 0xc5, 0x9f,
        0x2a, 0x11,
    ];
    ...
    let rb = sig.r().to_bytes();
    let sb = sig.s().to_bytes();
...
mod tests {
    ...
    #[test]
    fn test_tamper_msg() {
        let (_msg, px, py, sr, ss, _) = generate_test_vector().unwrap();
        let bad = b"tampered message";
        assert!(
            !verify_ecdsa_with_commitment(bad, &px, &py, &sr, &ss)
                .unwrap()
                .signature_valid
        );
    }
    ...
    fn test_tamper_sig() {
        ...
        sr[0] ^= 0x01;
        assert!(
            !verify_ecdsa_with_commitment(&msg, &px, &py, &sr, &ss)
                .unwrap()
                .signature_valid
        );
    }
    ...
    fn test_wrong_key() {
        let sk2b: [u8; 32] = [
            0x11, 0x2a, 0x9f, 0xc5, 0x3d, 0x6e, 0x8b, 0xaa, 0x42, 0x1f, 0x5c, 0x3b, 0x7e, 0x2d,
            0x8a, 0xcd, 0x06, 0x4f, 0x1d, 0x9e, 0xbb, 0x2a, 0x3f, 0x7c, 0x89, 0x6e, 0x5b, 0x2d,
            0x17, 0x3f, 0x4a, 0x8b,
        ];
        let sk2 = SecretKey::from_slice(&sk2b).unwrap();
        let pk2 = PublicKey::from(&sk2);
        let ep = pk2.to_sec1_point(false);
        let pb = ep.as_ref();
        ...
        assert!(
            !verify_ecdsa_with_commitment(&msg, &opx, &opy, &sr, &ss)
                .unwrap()
                .signature_valid
        );
    }
