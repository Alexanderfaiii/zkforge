...
fn main() -> anyhow::Result<()> {
  ...
  match cli.command {
    ...
    Commands::Prove { input, witness, onchain, rpc } => cmd_prove(&input, witness.as_ref(), onchain, &rpc),
    Commands::ProveNative { input, witness } => cmd_prove_native(&input, witness.as_ref()),
    Commands::Deploy { input, rpc, private_key, chain_id, format } => cmd_deploy(&input, &rpc, private_key.as_deref(), chain_id, &format),
    Commands::Bench { input } => cmd_bench(&input),
    Commands::Test { input } => cmd_test(&input),
    Commands::Init { name, template } => cmd_init(&name, &template),
    Commands::NlTranslate { description, output } => cmd_nl_translate(&description.join(" "), output.as_ref()),
    Commands::Plonk { input, witness, srs_size } => cmd_plonk(&input, witness.as_ref(), srs_size),
    Commands::Fold { input, steps, witnesses } => cmd_fold(&input, steps, witnesses.as_ref()),
    Commands::Zkml { model, input, model_file } => cmd_zkml(&model, input.as_deref(), model_file.as_ref()),
    Commands::Shield { input, output, public_vars } => cmd_shield(&input, &output, public_vars.as_deref()),
  }
}

fn cmd_compile(input: &PathBuf, output_dir: &PathBuf) -> anyhow::Result<()> {
  let source = fs::read_to_string(input)?;
  let filename = input.file_stem().and_then(|s| s.to_str()).unwrap_or("unknown");
  ...
  let readme = format!(
    ...
    compiled.name, compiled.info.proof_system.name()
  );
  ...
  println!(" 📊 {} constraints, {} signals", compiled.info.num_constraints, compiled.info.num_signals);
...
fn cmd_info(input: &PathBuf) -> anyhow::Result<()> {
  ...
  println!(" Gas (est):    ~{} gas", compiled.info.num_constraints * 200 + 250_000);
  Ok(())
}
  ...
  if input.is_dir() {
    ...
    for entry in fs::read_dir(input)? {
      ...
      if path.extension().and_then(|s| s.to_str()) == Some("zkf") {
        ...
        total_gas += gas;
        files += 1;
        println!(" {:<25} {:>5} constraints {:>4} signals {:>8} gas {:>6}μs",
          format!("{}.zkf", name), compiled.info.num_constraints,
          compiled.info.num_signals, format_gas(gas), elapsed.as_micros());
      }
    }
    let total_time = start.elapsed();
    println!();
    println!(" ─────────────────────────────────────────────────────");
    println!(" TOTAL: {} files {} constraints {} total gas", files, total_constraints, format_gas(total_gas));
    println!(" Compile time: {:.2}ms | Avg: {:.0}μs/file",
      total_time.as_secs_f64() * 1000.0,
      if files > 0 { total_compile_ns as f64 / (files as f64 * 1000.0) } else { 0.0 });
  } else {
    let source = fs::read_to_string(input)?;
    ...
    println!(" Throughput:   {:.0} constraints/ms",
...
fn cmd_test(input: &PathBuf) -> anyhow::Result<()> {
  ...
  let checks = [
    ("Has pragma declaration", compiled.circom.contains("pragma circom")),
    ("Has main template", compiled.circom.contains("template ZKForgeMain")),
    ("Has component main", compiled.circom.contains("component main")),
    ("Has public signals", compiled.circom.contains("public [")),
    ("Has verifier contract", compiled.verifier.contains("contract ZKForgeVerifier")),
    ("Has verifyProof function", compiled.verifier.contains("verifyProof")),
    ("Has pairing library", compiled.verifier.contains("library Pairing")),
    ("All asserts connect to output", compiled.circom.contains("valid ===")),
  ];
  let mut passed = 0;
  let mut failed = 0;
  for (check, ok) in &checks {
    if *ok { println!(" ✅ {}", check); passed += 1; }
    else { println!(" ❌ {}", check); failed += 1; }
  }
  let constraint_ok = compiled.info.num_constraints > 0;
  if constraint_ok { println!(" ✅ Has constraints ({})", compiled.info.num_constraints); passed += 1; }
  else { println!(" ❌ No constraints"); failed += 1; }
  let signal_ok = compiled.info.num_signals >= compiled.info.num_inputs;
  if signal_ok { println!(" ✅ Signal count valid ({})", compiled.info.num_signals); passed += 1; }
  else { println!(" ❌ Signal count < input count"); failed += 1; }

  println!();
  println!(" ─────────────────────────────────────────");
  println!(" Results: {} passed, {} failed", passed, failed);
  if failed > 0 { anyhow::bail!("Validation failed with {} error(s)", failed); }
  println!(" ✅ Circuit validated successfully");
  Ok(())
}

fn cmd_prove(input: &PathBuf, witness: Option<&PathBuf>, _onchain: bool, _rpc: &str) -> anyhow::Result<()> {
  let source = fs::read_to_string(input)?;
  let name = input.file_stem().and_then(|s| s.to_str()).unwrap_or("circuit");
  let start = Instant::now();
  ...
  fs::write(work_dir.join(format!("{}Verifier.sol", name)), &compiled.verifier)?;
...
"###;
  ...
  let result = std::process::Command::new("node")
    ...
    .env("NODE_PATH", std::env::var("NODE_PATH").unwrap_or_else(|_| {
      if cfg!(windows) { r"C:\Users\PC\AppData\Roaming\npm\node_modules".to_string() }
      else { "/usr/local/lib/node_modules".to_string() }
    }))
  ...
  match &result {
    ...
    Ok(out) => {
      ...
      for line in stderr.lines().take(5) {
        let t = line.trim();
        if !t.is_empty() {
          if !t.contains("npm warn") && !t.contains("deprecat") {
            println!(" stderr: {}", t);
          }
        }
      }
      println!();
      println!(" 📋 Manual run: cd {} && node prove.js {} {}", work_dir.display(), name, name);
    }
    Err(e) => {
...
fn cmd_prove_native(input: &PathBuf, witness: Option<&PathBuf>) -> anyhow::Result<()> {
  ...
  let cs = compiled.cs.as_ref().ok_or_else(|| anyhow::anyhow!("No constraint system"))?;
  ...
  println!("     PK: {} bytes, VK: {} bytes (took {:.2}s)", params.pk.len(), params.vk.len(), setup_time.as_secs_f64());

  println!(" [3/4] Generating proof...");
  let mut private_inputs: std::collections::HashMap<String, num_bigint::BigUint> = std::collections::HashMap::new();
  ...
  if let Some(w) = witness {
    ...
    for sig in &cs.signals {
      let val: u64 = if sig.name.contains("age") { 25 }
      else if sig.name.contains("credit") { 750 }
      else if sig.name.contains("min") || sig.name.contains("threshold") {
        if sig.name.contains("age") { 18 } else { 700 }
      } else if sig.name.contains("balance") { 5000000 }
      else if sig.name.contains("required") { 1000000 }
      else if sig.name.contains("total") { 10000000 }
      else if sig.name.contains("root") { 42 }
      else { 0 };
    ...
    for c in &cs.constraints {
      ...
      for (name, _) in a_lc.iter().chain(b_lc.iter()).chain(c_lc.iter()) {
        private_inputs.entry(name.clone()).or_insert_with(|| bu(0));
      }
    }
  }

  let proof = groth16_native::prove(&r1cs, &params, private_inputs, HashMap::<String, num_bigint::BigUint>::new())
    .map_err(|e| anyhow::anyhow!("{}", e))?;
  let prove_time = start.elapsed();
  println!("     Proof: {} bytes (took {:.2}s total)", proof.proof.len(), prove_time.as_secs_f64());
  ...
  if verified {
    ...
    fs::write(out_dir.join("public.json"), serde_json::to_string_pretty(&pub_strs)?)?;
    ...
    if let Some(ref coords) = params.vk_coords {
      ...
      println!(" 📜 Verifier:   {} ({} bytes)", out_dir.join(format!("{}.sol", contract_name)).display(), sol.len());
    }
...
fn parse_witness_val(v: &serde_json::Value) -> num_bigint::BigUint {
  ...
  match v {
    ...
    serde_json::Value::String(s) => {
      ...
      } else { BigUint::from(s.parse::<u64>().unwrap_or(0)) }
    }
    _ => BigUint::from(0u64),
  }
}
...
fn term_to_lc(
  ...
  match term {
    Term::Signal(name) => (vec![(name.clone(), BigUint::from(1u64))], BigUint::from(0u64)),
    ...
    Term::Neg(inner) => {
      ...
      let neg_const = if constant > BigUint::from(0u64) { &m - &constant } else { BigUint::from(0u64) };
      (vars, neg_const)
    }
    Term::Add(l, r) => {
      let (lv, lc) = term_to_lc(l);
      let (rv, rc) = term_to_lc(r);
      let mut map: std::collections::HashMap<String, BigUint> = std::collections::HashMap::new();
      let m = zkforge_compiler::r1cs::field_modulus();
      for (name, coeff) in lv { *map.entry(name).or_insert_with(|| BigUint::from(0u64)) += coeff; }
      for (name, coeff) in rv { *map.entry(name).or_insert_with(|| BigUint::from(0u64)) += coeff; }
      (map.into_iter().map(|(n, c)| (n, c % &m)).collect(), (lc + rc) % &m)
    }
    Term::Sub(l, r) => {
      let (lv, lc) = term_to_lc(l);
      ...
      let neg_rc = if rc > BigUint::from(0u64) { &m - &rc } else { BigUint::from(0u64) };
      let mut map: std::collections::HashMap<String, BigUint> = std::collections::HashMap::new();
      for (name, coeff) in lv { *map.entry(name).or_insert_with(|| BigUint::from(0u64)) += coeff; }
      for (name, coeff) in rv { *map.entry(name).or_insert_with(|| BigUint::from(0u64)) += coeff; }
      (map.into_iter().map(|(n, c)| (n, c % &m)).collect(), (lc + neg_rc) % &m)
    }
    Term::Linear(terms) => {
...
fn generate_witness_json(circom_source: &str) -> String {
  ...
  for line in circom_source.lines() {
    ...
    if trimmed.starts_with("signal input ") {
      let sig_name = trimmed.strip_prefix("signal input ").unwrap_or("").trim_end_matches(';').to_string();
      if !first { json.push_str(",\n"); }
      first = false;
      let val = if sig_name.contains("age") { "25" }
      else if sig_name.contains("credit_score") || sig_name.contains("credit") { "750" }
      else if sig_name.contains("min_score") || sig_name.contains("threshold") { "700" }
      else if sig_name.contains("balance") { "5000000" }
      else if sig_name.contains("total_supply") { "10000000" }
      else if sig_name.contains("required_amount") { "1000000" }
      else if sig_name.contains("root") || sig_name.contains("hash") {
        "0x1234567890abcdef1234567890abcdef1234567890abcdef1234567890abcdef"
      } else if sig_name.contains("min_age") { "18" }
      else if sig_name.contains("proof") || sig_name.contains("path") { "0xabc" }
      else { "42" };
...
fn generate_prove_script(name: &str, info: &zkforge_compiler::constraints::CircuitInfo) -> String {
  ...
  let pot_size = if constraints < 100 { 8 } else if constraints < 1000 { 10 } else { 12 };
  ...
  format!(r#"# ZKForge Prove Pipeline — {name} circuit
...
"#, name = name, constraints = constraints, pot_size = pot_size, pot_file = pot_file, pot_url = pot_url)
}
...
fn cmd_init(name: &str, template: &str) -> anyhow::Result<()> {
  let template_content = match template {
    ...
    _ => anyhow::bail!("Unknown template: {}. Available: age, nft, credit, balance", template),
  };
...
fn cmd_deploy(
  ...
  let cs = compiled.cs.as_ref().ok_or_else(|| anyhow::anyhow!("No constraint system"))?;
  let mut r1cs = R1CSSystem::new();
  for sig in &cs.signals { r1cs.alloc_witness(&sig.name); }
  for c in &cs.constraints {
    let (a_lc, a_const) = term_to_lc(&c.a);
    let (b_lc, b_const) = term_to_lc(&c.b);
    let (c_lc, c_const) = term_to_lc(&c.c);
  ...
  let params = groth16_native::setup(&r1cs).map_err(|e| anyhow::anyhow!("{}", e))?;
  println!("     PK: {} bytes, VK: {} bytes", params.pk.len(), params.vk.len());
  ...
  for sig in &cs.signals {
    ...
    else if sig.name.contains("min") || sig.name.contains("threshold") { if sig.name.contains("age") { 18 } else { 700 } }
    else if sig.name.contains("balance") { 5000000 }
    else if sig.name.contains("required") { 1000000 }
    else if sig.name.contains("total") { 10000000 }
    else { 0 };
  ...
  let proof = groth16_native::prove(&r1cs, &params, priv_inp, HashMap::<String, num_bigint::BigUint>::new()).map_err(|e| anyhow::anyhow!("{}", e))?;
  let verified = groth16_native::verify(&params, &proof).map_err(|e| anyhow::anyhow!("{}", e))?;
  if !verified { anyhow::bail!("Proof verification failed — cannot deploy"); }
  ...
  let sol_path = deploy_dir.join("src").join(format!("{}.sol", contract_name));
  ...
  println!(" Chain:    {} (ID: {})", if chain_id == 31337 { "Anvil/Local" } else if chain_id == 11155111 { "Sepolia" } else if chain_id == 17000 { "Holesky" } else { "Custom" }, chain_id);
  println!(" RPC:     {}", rpc);
  println!(" Proof:    {} bytes", proof.proof.len());
  ...
  if format == "hardhat" {
    ...
    println!("  cd {} && npx hardhat run scripts/deploy.js --network <network>", deploy_dir.display());
  } else {
    println!(" Deploy with Foundry:");
    let pk_env = if private_key.is_some() { "" } else { "PRIVATE_KEY=<key> " };
    println!("  cd {} && {}forge script script/Deploy.s.sol --rpc-url {} --broadcast", deploy_dir.display(), pk_env, rpc);
    println!();
    println!(" Or one-liner with forge create:");
    println!("  cd {} && {}forge create --rpc-url {} --private-key $PRIVATE_KEY src/{}.sol:{}", deploy_dir.display(), pk_env, rpc, contract_name, contract_name);
  }
...
fn cmd_nl_translate(description: &str, output_dir: Option<&PathBuf>) -> anyhow::Result<()> {
  ...
  let result = translate(description)
  ...
  for (i, tv) in result.test_inputs.iter().enumerate() {
    let status = if tv.should_pass { "✅ SHOULD PASS" } else { "❌ SHOULD FAIL" };
  ...
  if let Some(out_dir) = output_dir {
    ...
    let compiled = zkforge_compiler::compile(
      &result.zkf_source,
      &format!("{}.zkf", result.circuit_name),
    )?;
...
fn cmd_plonk(input: &PathBuf, witness_path: Option<&PathBuf>, srs_size: usize) -> anyhow::Result<()> {
  ...
  let (pk, vk) = plonk_prover::setup(&rcs, &srs)
    .map_err(|e| anyhow::anyhow!("Plonk setup: {}", e))?;
  ...
  fn json_to_biguint(v: &serde_json::Value) -> BigUint {
    ...
    if let Some(n) = v.as_u64() { return BigUint::from(n); }
    if let Some(n) = v.as_i64().filter(|&n| n >= 0) { return BigUint::from(n as u64); }
    if let Some(s) = v.as_str() {
      if let Ok(n) = s.parse::<u64>() { return BigUint::from(n); }
      // Try BigUint from decimal string
      if let Some(n) = num_bigint::BigUint::parse_bytes(s.as_bytes(), 10) { return n; }
    }
    BigUint::from(0u64)
  }
    ...
    if let Some(flat) = witness_json.as_object() {
      if !flat.is_empty() && !flat.contains_key("private") && !flat.contains_key("public") {
        for (k, v) in flat {
          ...
          if rcs.public_vars.iter().any(|vi| {
            rcs.vars.get(k).map(|vr| vr.0 == *vi).unwrap_or(false)
          }) {
...
fn cmd_fold(input: &PathBuf, steps: u64, _witnesses_path: Option<&PathBuf>) -> anyhow::Result<()> {
  ...
  // Real Groth16 setup
  let params = groth16_native::setup(&r1cs)
    .map_err(|e| anyhow::anyhow!("Groth16 setup: {}", e))?;
  ...
  let start = std::time::Instant::now();
  let valid = recursive_prover::verify_folded(&proof)
    .map_err(|e| anyhow::anyhow!("Verify: {}", e))?;
  let verify_time = start.elapsed().as_secs_f64();
  println!("     {}", if valid { "✅ VALID" } else { "❌ INVALID" });
  println!("     Verify time: {:.4}s (same for 1 or {} steps!)", verify_time, steps);
  ...
  println!("     Gas saved vs {} separate verifications: {}K gas", steps, gas.gas_saved / 1000);
  println!("     Verification time: {}ms (O(1) regardless of steps)", gas.verification_time_ms);
  ...
  println!(" Verify cost: {}K gas (O(1) regardless of N)", gas.gas_cost / 1000);
  ...
  let verifier_path = PathBuf::from("output").join(format!("{}_recursive_verifier.sol", filename));
...
fn cmd_zkml(model_type: &str, input_str: Option<&str>, model_file: Option<&PathBuf>) -> anyhow::Result<()> {
  ...
  let model = if let Some(mf) = model_file {
    ...
    match model_type {
      ...
      other => anyhow::bail!("Unknown model type: {}. Use 'mnist' or 'credit', or --model-file for custom", other),
    }
  };

  // Parse input
  let input: Vec<i32> = if let Some(input_s) = input_str {
    input_s.split(',').map(|s| s.trim().parse::<i32>()).collect::<Result<Vec<_>, _>>()?
  } else {
  ...
  println!(" Architecture: {} → {} → ... → {}", model.input_dim,
    if model.layers.len() > 1 {
      let first = &model.layers[0];
      match first { zkml::ZKLayer::Dense { weights, .. } => format!("{}", weights.len()),
        _ => "?".to_string(),
      }
    } else { "?".to_string() },
    model.output_dim);
  ...
  let start = std::time::Instant::now();
  let (proof, _cs) = zkml::prove_inference(&model, &input)
    .map_err(|e| anyhow::anyhow!("zkML: {}", e))?;
  ...
  println!("     Output: {}", proof.output.iter()
    .map(|v| format!("{}", v))
    .collect::<Vec<_>>()
    .join(", "));
  ...
  match &proof.groth16_proof {
    ...
    None => println!("     ⚠️ Groth16 ZK Proof: NOT GENERATED (circuit too complex / witness mismatch)"),
  }
  ...
  for (i, count) in proof.layer_constraints.iter().enumerate() {
    let layer_type = if i == 0 { "Input" }
      else if i == proof.layer_constraints.len() - 1 { "Output" }
      else { "Hidden" };
...
fn convert_to_r1cs(compiled: &zkforge_compiler::CompileOutput) -> zkforge_compiler::r1cs::R1CSSystem {
  use zkforge_compiler::r1cs::R1CSSystem;
...
fn extract_terms(
  term: &zkforge_compiler::constraints::Term,
  signals: &[zkforge_compiler::constraints::Signal],
) -> Vec<(String, u64)> {
  use zkforge_compiler::constraints::Term;
  match term {
    ...
    Term::Linear(terms) => terms.iter().map(|(c, s)| {
      (s.clone(), c.parse::<u64>().unwrap_or(1))
    }).collect(),
    Term::Neg(inner) => extract_terms(inner, signals)
      .into_iter()
      .map(|(n, c)| (n, -(c as i64) as u64))
      .collect(),
    Term::Add(l, r) => {
      let mut result = extract_terms(l, signals);
      result.extend(extract_terms(r, signals));
      result
    }
    Term::Sub(l, r) => {
      let mut result = extract_terms(l, signals);
      let right = extract_terms(r, signals);
      for (n, c) in right {
...
fn cmd_shield(input: &PathBuf, output_dir: &PathBuf, public_vars: Option<&str>) -> anyhow::Result<()> {
  use zkforge_compiler::auto_shield;

  println!("🛡️ Auto-Shield Solidity — Make Any Contract Private\n");

  let source = fs::read_to_string(input)?;
  let contract_name = input.file_stem()
    .and_then(|s| s.to_str())
    .unwrap_or("Unknown");

  println!(" Parsing: {}...", contract_name);
  let contract = auto_shield::parse_solidity(&source)
  ...
  println!("     {} functions shielded", shield.stats.num_shielded_functions);
  ...
  let sol_path = output_dir.join("src").join(format!("{}.sol", shield.shielded_name));
  fs::write(&sol_path, &shield.source)?;
  println!("     {}", sol_path.display());

  for (func_name, circuit) in &shield.circuits {
    let circ_path = output_dir.join("circuits").join(format!("shield_{}.zkf", func_name));
  ...
  println!(" Gas per call: ~{}K", shield.stats.estimated_gas_per_call / 1000);
  println!();
  println!(" 📁 Output: {}", output_dir.display());
  println!();
